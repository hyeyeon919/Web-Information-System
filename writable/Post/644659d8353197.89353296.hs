...
insert k v (FunMap f) = FunMap (\key -> if key == k then Just v else f key)
...
delete k (FunMap f) = FunMap (\key -> if key == k then Nothing else f key)
...
instance Functor (FunMap k) where
  fmap f (FunMap m) = FunMap (\k -> fmap f (m k))
...
changeCurrency exchangeRate = fmap exchangeRate
...
convertMarks = fmap convertMark
  where
    convertMark :: Int -> Char
    convertMark mark
      | mark >= 84 && mark <= 100 = 'A'
      | mark >= 67 && mark <= 83 = 'B'
      | mark >= 50 && mark <= 66 = 'C'
